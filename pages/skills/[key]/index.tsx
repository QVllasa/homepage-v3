...
function SectionItem({section}: { section: any }) {
    const sectionTitle = useTranslatedContent(section.title);
    const sectionDescription = useTranslatedContent(section.description);

    // Determine icon based on translated title
    const getIcon = (title: string) => {
        const lowerTitle = title.toLowerCase();
        if (lowerTitle.includes('web') || lowerTitle.includes('anwendung')) {
            return (
        ...
        } else if (lowerTitle.includes('teaching') || lowerTitle.includes('lehre')) {
            return (
                <svg className="h-8 w-8 text-white dark:text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        } else if (lowerTitle.includes('website')) {
            return (
                <svg className="h-8 w-8 text-white dark:text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
        ...
        } else if (lowerTitle.includes('analytics') || lowerTitle.includes('business')) {
            return (
                <svg className="h-8 w-8 text-white dark:text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
        ...
        } else if (lowerTitle.includes('research') || lowerTitle.includes('forschung')) {
            return (
                <svg className="h-8 w-8 text-white dark:text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    return (
        <div className="relative bg-white dark:bg-slate-800 rounded-3xl border border-gray-200 dark:border-slate-600 p-8 overflow-hidden">
            {/* Content */}
                ...
                {section.keys && (
                    <div className="space-y-6">
                        {section.keys.map((key, keyIdx) => (
                            <KeyItem key={keyIdx} keyItem={key}/>
                        ))}
                    </div>
                )}
            </div>
...
// Separate component for key items to properly use hooks
function KeyItem({keyItem}: { keyItem: any }) {
    const keyTitle = useTranslatedContent(keyItem.title);
    const keyDescription = useTranslatedContent(keyItem.description);

    return (
        <div className="flex items-start space-x-4 p-4 rounded-xl bg-gray-50 dark:bg-slate-700/50">
...
export default function Skill() {
    ...
    // Call hooks unconditionally, but handle the case where skill might be undefined
    const skillTitle = useTranslatedContent(skill?.title || {en: '', de: ''});
    const skillDescription = useTranslatedContent(skill?.description || {en: '', de: ''});
    const skillSlug = useTranslatedContent(skill?.slug || {en: '', de: ''});
